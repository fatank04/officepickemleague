#!/bin/zsh
# SHIFT CHANGE hero ad — Seedance 2.0 comparison cut. Identical timeline/audio to
# assemble-hero.sh; only the video sources differ (promo-assets/seedance/sd-*.mp4).
set -e
...
"$FF" -y -v error \
  ...
  "$PA/promo-hero-shift-change-seedance.mp4"
echo "--- done ---"; ls -la "$PA/promo-hero-shift-change-seedance.mp4"
